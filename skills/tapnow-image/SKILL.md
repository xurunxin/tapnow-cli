...
Check `library search` and `roles list/get` for reusable approved references before generating replacements. `library attach` or `roles attach` materializes pinned source nodes and provenance into a new workflow file. For an existing project, inspect `projects overview` and `comments inbox` and use `projects checkout` to preserve existing outputs and remote node IDs.
