...
Use `library search` and `roles list/get` to find approved characters, scenes and sounds. Attach the chosen asset or role to a new workflow version; `roles attach` pins its revision and description. Choose reference mode for reusable identity, or first-frame mode when the image defines the opening frame. For ongoing projects, read `projects overview` and `comments inbox`; group shots and retain selected outputs with source provenance for human review.
